            ...
            async move {
                ...
                let metrics_pub_key = match client.get(bridge_request_url).send().await {
                    Ok(response) => {
                        ...
                        match Ed25519PublicKey::from_bytes(&metrics_bytes) {
                            Ok(pubkey) => {
                                ...
                                let mut metrics_keys_write = metrics_keys.write().unwrap(); // Write lock for cache update
                    ...
                    Err(_) => {
                        ...
                        let metrics_keys_read = metrics_keys.read().unwrap(); // Read lock for the cache
