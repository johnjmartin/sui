...
pub struct StorageConnectionArgs {
    /// How long to wait for a snapshot file to be downloaded (defaults to no timeout).
    #[arg(long)]
    pub snapshot_timeout_ms: Option<u64>,

    /// How long to wait while establishing a connection to the snapshot store (defaults to no timeout).
    #[arg(long)]
